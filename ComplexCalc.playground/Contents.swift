...
class Calculator {
    ...
    func add(lhs: (Int, Int), rhs: (Int, Int)) -> (Int, Int) {
        let xValue = lhs.0 + rhs.0
        let yValue = lhs.1 + rhs.1
        let point: (Int, Int) = (xValue, yValue)
        return point
    }
    
    func add(lhs: Dictionary<String,Int>, rhs: Dictionary<String,Int>) -> Dictionary<String,Int> {
        let xValue = lhs["x"]! + rhs["x"]!
        let yValue = lhs["y"]! + rhs["y"]!
        let point: [String: Int] = ["x": xValue, "y": yValue]
        return point
    }
